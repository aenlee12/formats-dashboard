...
def main():
    ...
    if test_mode:
        ...
        weekly_full['Чистая_выручка'] = (
            weekly_full['Сумма_выручки'] * (1 - weekly_full['Среднее_списаний']/100)
        )
        ...
        st.subheader(
            "📋 Усреднённая аналитика до/во время теста "
            "(только полные недели)"
        )
        st.markdown(f"""
- **Выручка (среднее)**: {rev_pre:.0f} → {rev_post:.0f} ₽ ({(rev_post/rev_pre-1)*100:.1f}% изм.)
- **% списаний (среднее)**: {waste_pre:.1f}% → {waste_post:.1f}% ({(waste_post-waste_pre):+.1f} п.п.)
- **Чистая выручка**: {net_pre:.0f} → {net_post:.0f} ₽ ({(net_post/net_pre-1)*100:.1f}% изм.)
"""
        )

        # Общие графики теста
        fig_total_rev = px.line(
            weekly_full, x='Неделя', y='Сумма_выручки', markers=True,
        ...
        fig_total_waste = px.line(
            weekly_full, x='Неделя', y='Среднее_списаний', markers=True,
    ...
    fig_heat.data[0].hovertemplate = (
        'Категория=%{y}<br>День=%{x}<br>Выручка=%{text:.0f}<extra></extra>'
    )
