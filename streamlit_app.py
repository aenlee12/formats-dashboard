...
@st.cache_data
def load_df(file):
    """Загрузить CSV/Excel и переименовать столбцы в стандартные."""
    name = file.name.lower()
    if name.endswith('.csv'):
        df = pd.read_csv(file)
    else:
        df = pd.read_excel(file, header=0)
    # унифицируем имена в lower для сопоставления
...
def prepare_data(files):
    """Из двух файлов (доля + выручка) собрать единый DF."""
    df_share = df_rev = None
    for f in files:
        df = load_df(f)
        # смотрим на стандартизированные названия
    ...
    if df_share is None or df_rev is None:
        raise ValueError("Нужно два файла: один с колонкой 'Доля списаний и ЗЦ' и один с 'Выручка'")
...
def display_format_section(title, files):
    st.header(title)
    if len(files) != 2:
        st.info("Загрузите ровно два файла:\n"
    ...
    # пороги
    share_thr = st.slider(f"Порог % списаний ({title})",      0.0, 100.0, 20.0)
    ...
    # распрямляем MultiIndex колонок
    ...
    st.subheader("Таблица: по неделям и дням недели")
    ...
    fig_waste = px.line(wk, x='Неделя', y='Доля списаний и ЗЦ', color='Категория',
                        markers=True, title="Средневзвешенная % списаний")
    st.plotly_chart(fig_waste, use_container_width=True)

    # 3) Heatmap выручки: категория vs день недели
    heat = df.pivot_table(
        index='Категория', columns='DayOfWeek',
        values='Выручка', aggfunc='sum'
    )
    st.subheader("Heatmap выручки по дню недели")
    fig_heat = px.imshow(
        heat,
        labels=dict(x="День недели", y="Категория", color="Выручка"),
        aspect="auto", title="Выручка (категория vs день)"
    )
    st.plotly_chart(fig_heat, use_container_width=True)

    # Экспорт всех трёх табличек в Excel
    buf = BytesIO()
...
def main():
    ...
    juk_files  = st.sidebar.file_uploader(
        "Формат Жук: файлы «% списаний» и «Выручка»",
        type=['csv','xlsx'], accept_multiple_files=True, key='juk'
    )
    test_files = st.sidebar.file_uploader(
        "Тестовый формат: файлы «% списаний» и «Выручка»",
        type=['csv','xlsx'], accept_multiple_files=True, key='test'
    )
